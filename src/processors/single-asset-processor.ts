...
import {
    SingleAssetMessageProcessingEvent,
    SingleAssetMessageProcessingStats
} from "../schema/schema.oapp_single_asset.js";
...
type SingleAssetOAppProcessor = typeof single_asset_oapp_movement;
...
export function singleAssetProcessor(
    supportedChainId: SupportedAptosChainId,
    startVersion: number,
    baseProcessor: SingleAssetOAppProcessor
) {
    baseProcessor
        .bind({ startVersion })
        .onEventMessageProcessingEvent(async (event, ctx) => {
            ...
            const eventEntity = new SingleAssetMessageProcessingEvent({
...
async function getOrCreateStats(store: Store, timestamp: bigint): Promise<SingleAssetMessageProcessingStats> {
    const STATS_ID = "global";
    let stats = await store.get(SingleAssetMessageProcessingStats, STATS_ID);

    if (!stats) {
        stats = new SingleAssetMessageProcessingStats({
            id: STATS_ID,
...
export class SingleAssetReader {
    constructor(private store: Store) {}

    async getStats(): Promise<SingleAssetMessageProcessingStats | undefined> {
        return this.store.get(SingleAssetMessageProcessingStats, "global");
    }

    async getEventByID(id: string): Promise<SingleAssetMessageProcessingEvent | undefined> {
        return this.store.get(SingleAssetMessageProcessingEvent, id);
    }

    // TODO: investigate if there's a way to "limit" or create an issue for it
    async getEventsByType(messageType: bigint, limit: number = 100): Promise<SingleAssetMessageProcessingEvent[]> {
        return this.store.list(SingleAssetMessageProcessingEvent, [
            { field: "message_type", op: "=", value: messageType }
        ]);
    }

    async getEventsByTraceCode(traceCode: number, limit: number = 100): Promise<SingleAssetMessageProcessingEvent[]> {
        return this.store.list(SingleAssetMessageProcessingEvent, [
            { field: "trace_code", op: "=", value: traceCode }
        ]);
    }

    async getLatestEvents(limit: number = 100): Promise<SingleAssetMessageProcessingEvent[]> {
        return this.store.list(SingleAssetMessageProcessingEvent, []);
    }
}
